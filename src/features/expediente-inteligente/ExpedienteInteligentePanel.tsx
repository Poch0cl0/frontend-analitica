...
import type { ExpedienteTab } from './types';
...
export interface ExpedienteInteligentePanelProps {
  pacienteId?: number | null;
  embedded?: boolean;
  onClose?: () => void;
}

export default function ExpedienteInteligentePanel({
  pacienteId: initialPacienteId,
  embedded = false,
  onClose,
}: ExpedienteInteligentePanelProps) {
  ...
  useEffect(() => {
    setSelectedPacienteId(initialPacienteId ?? null);
  }, [initialPacienteId]);
  ...
  const shellClass = embedded
    ? 'bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col min-h-[calc(100vh-7rem)]'
    : 'bg-white rounded-2xl max-w-5xl w-full shadow-2xl border border-gray-100 flex flex-col animate-zoom-in';

  const contentClass = embedded
    ? 'p-6 flex-1 overflow-y-auto'
    : 'p-6 max-h-[65vh] overflow-y-auto';
      ...
      >
        <div className="text-white">
          <h3 className="font-extrabold text-lg">
            {embedded ? 'Predicción — Expediente Inteligente' : 'Expediente Inteligente'}
          </h3>
          <p className="text-xs text-fuchsia-200">Predicción · Triaje · Recomendaciones</p>
        </div>
        {onClose && (
          <button type="button" onClick={onClose} className="text-fuchsia-200 hover:text-white transition-colors">
            <X className="w-5 h-5" />
          </button>
        )}
      </div>
      ...
      <div className="px-6 py-3 border-b border-gray-100 bg-gray-50/50">
        {selectedPaciente ? (
          ...
          <div ref={searchRef} className="relative max-w-xl">
            ...
            />
            {showDropdown && (
              <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl max-h-60 overflow-y-auto divide-y divide-gray-100">
                {loadingPatients ? (
                  ...
                  <div className="p-4 text-center text-xs font-medium text-red-500">{patientError}</div>
        ...
        {TABS.map((tab) => (
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3.5 text-sm font-bold transition-colors relative ${
              activeTab === tab.key ? 'text-fuchsia-900' : 'text-gray-400 hover:text-gray-600'
            }`}
          >
            <span>{tab.icon}</span>
            <span>{tab.label}</span>
      ...
      <div className={contentClass}>
        {!selectedPacienteId ? (
          <div className="py-12 text-center text-gray-400 text-sm">
            Seleccione un paciente para ver la información de{' '}
